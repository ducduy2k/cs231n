...
def svm_loss(x, y):
    ...
    margin = dx.sum(axis = 1)
    dx[np.arange(x.shape[0]),y] = -margin
    dx = dx / x.shape[0]
    # *****END OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
    ###########################################################################
    #                             END OF YOUR CODE                            #
...
def softmax_loss(x, y):
    ...
    num_input = x.shape[0]
    softmax_matrix = np.exp(x)
    softmax_matrix = softmax_matrix/softmax_matrix.sum(axis = 1).reshape((-1,1))
    loss = np.sum(-np.log(softmax_matrix[np.arange(x.shape[0]),y]) )/ num_input
    dx = softmax_matrix
